...
const useResize = (
  ...
  const showNuxtContentSlot = ref(false)
  ...
  watch(
    ...
    () => {
      ...
      const nuxtContent = false;

      if(!nuxtContent){
        inputWrapperStyle.width = state.resizedWidth;
        resizeOperateStyle.display = 'initial';
        showPreviewWrapper.value = false;
        showNuxtContentSlot.value = true;
      }
    },
    {
      immediate: true
  ...
  return { inputWrapperStyle, resizeOperateStyle, showPreviewWrapper,showNuxtContentSlot };
};
